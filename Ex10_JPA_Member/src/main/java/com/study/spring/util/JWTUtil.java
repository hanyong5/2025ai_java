...
public class JWTUtil {
	...
	public static String generateToken(Map<String, Object> claims, int i) {
		...
		String jwtStr = Jwts.builder().setHeader(Map.of("typ", "JWT")).setClaims(claims)
				.setIssuedAt(Date.from(ZonedDateTime.now().toInstant()))
				.setExpiration(Date.from(ZonedDateTime.now().plusMinutes(i).toInstant())).signWith(key).compact();
